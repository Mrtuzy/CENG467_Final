...
To train the CfC network, we need ground truth importance scores ($p_{target}$) for each $u_i$. We obtain these using a "leave-one-out" approach with a powerful Teacher model (LLaMA-3-8B-Instruct).

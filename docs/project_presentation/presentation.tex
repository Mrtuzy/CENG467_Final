...
\begin{frame}{Why Simulations Before Training?}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}
      ...
      \item If $\rho \approx 0$, entropy adds no signal and training is wasted.
      \item $\beta$, $\Delta t_{\min}$, $\tau$ affect faithfulness but are hard to tune end-to-end.
    \end{itemize}

    \textbf{The strategy}
    \begin{itemize}
      \item Seven model-free simulations, each answering one design question.
      \item Each outputs a concrete config value or a go/no-go threshold.
      \item Only if all sims pass $\rightarrow$ commit to training.
    \end{itemize}

    \column{0.46\textwidth}
    \includegraphics[width=\linewidth]{s3_proxy_quality_limit.png}
    \vspace{0.2em}
    \scriptsize S3: entropy-$\Delta t$ helps only when $\rho > \rho^* \approx 0.1$.
    Verify on real QReCC before training.
  \end{columns}
\end{frame}

\begin{frame}{Simulation Suite: 7 Design Questions Answered}
  \small
  \begin{tabular}{p{0.03\textwidth}p{0.38\textwidth}p{0.49\textwidth}}
    ...
    S2 & Which $\beta^*$ and $\Delta t_{\min}^*$ maximise importance recovery? & $\beta^*=1.0$,\; $\Delta t_{\min}^*=0.05$ \\
    S3 & What $\rho$ threshold makes entropy useful? (go/no-go) & $\rho^* \approx 0.1$ vs.\ uniform; $\rho \geq 0.5$ beats cosine \\
    S4 & Does LOO-KL measure raw importance or marginal necessity? & KL $=$ marginal necessity; correct pruning target \\
    S5 & Which $\Delta t$ mapping is robust to outlier surprisal? & Rank normalisation most robust \\
    S6 & What compression--fidelity trade-off does the system achieve? & 40\% compression at 90\% fidelity; uniform 9\%, cosine 31\% \\
    \bottomrule
  \end{tabular}
\end{frame}

\begin{frame}{Headline Result: Simulation S6 --- Fidelity vs.\ Compression}
  \centering
  \includegraphics[width=0.74\linewidth]{s6_endtoend_pruning.png}

  \small
  Entropy-CfC: \textbf{40\% compression at 90\% fidelity}.
  Uniform: 9\%.\enspace Cosine: 31\%.\enspace Random: 10\%.\enspace $\tau^* \approx 0.48$.
\end{frame}

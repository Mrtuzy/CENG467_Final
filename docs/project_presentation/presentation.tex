...
\usepackage{booktabs}
\usepackage{graphicx}
\graphicspath{{../../simulations/cfc_proof/figures/}}
\usepackage{amsmath}
\usepackage{array}
\usepackage{hyperref}
\usepackage{tikz}
...
\begin{frame}{Simulation Suite: a Roadmap}
  ...
  \vspace{0.4em}
  \footnotesize The next slides walk these in order: validity (S0--S1), design (S2, S5), label (S4), the critical test (S3), and the payoff (S6).
\end{frame}

\begin{frame}{S0 --- Is the Commitment Lever Real?}
  \begin{columns}[c,onlytextwidth]
    \column{0.60\textwidth}
    \includegraphics[width=\linewidth]{s0_substrate_faithfulness.png}

    \column{0.38\textwidth}
    \small
    \textbf{Validity foundation.}
    \begin{itemize}
      \item Our analysis uses an LTC toy ($\varphi = 1 - e^{-\lambda \Delta t}$); we \emph{train} the ncps CfC.
      \item Both rise monotonically with $\Delta t$: real CfC tracks the analytic curve at $\rho = +0.94$.
      \item So a wider $\Delta t$ really does ``commit'' the state --- not an artefact of the toy model.
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{S1 --- Derived, Not Circular --- and It Saturates}
  \centering
  \includegraphics[width=0.96\linewidth]{s1_mechanism_analytic.png}

  \vspace{0.2em}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
    \textbf{Left:} commitment is a \emph{closed-form} function of $\Delta t$ --- a derivation, so the surprisal$\rightarrow\varphi$ link is not a tautology.
    \column{0.48\textwidth}
    \textbf{Right:} $d\varphi/dS$ decays --- past a point, more surprisal adds no discrimination (31.6\% of turns saturate). This motivates S5.
  \end{columns}
\end{frame}
...
\begin{frame}{S5 --- Which $\Delta t$ Mapping Survives Outliers?}
  \begin{columns}[c,onlytextwidth]
    \column{0.60\textwidth}
    \includegraphics[width=\linewidth]{s5_mapping_function_design.png}

    \column{0.38\textwidth}
    \small
    \textbf{Why it matters (from S1).}
    \begin{itemize}
      \item DistilGPT-2 surprisal is heavy-tailed: one outlier turn can monopolise a linear time budget.
      \item Under injected outliers, \textbf{rank} normalisation keeps the highest recovery.
      \item $\Rightarrow$ \texttt{build\_inputs.py} maps surprisal $\rightarrow \Delta t$ by rank.
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}{S4 --- What Does the Teacher Label Measure?}
  \begin{columns}[c,onlytextwidth]
    \column{0.58\textwidth}
    \includegraphics[width=\linewidth]{s4_loo_kl_redundancy.png}

    \column{0.40\textwidth}
    \small
    Teacher labels come from \textbf{leave-one-out KL} on Mistral output.
    \begin{itemize}
      \item KL tracks answer-impact (TV) at $\approx 0.97$ \emph{regardless} of redundancy (blue).
      \item Its link to \emph{raw} importance drops as turns repeat info (red, $0.81\!\to\!0.54$).
      \item So KL $=$ \emph{marginal} answer-necessity --- exactly the right target: redundant turns are safe to prune.
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{S6 --- Headline: Fidelity vs.\ Compression}
  \centering
  \includegraphics[width=0.72\linewidth]{s6_endtoend_pruning.png}

  \small
  Entropy-CfC: \textbf{40\% compression at 90\% fidelity}.
  Uniform: 9\%.\enspace Cosine: 31\%.\enspace Random: 10\%.\enspace $\Rightarrow \tau^* \approx 0.48$.
\end{frame}
...
\begin{frame}{Entropy-Driven CfC Pruning: Architecture}
  ...
  \begin{itemize}
    ...
    \item \textbf{Teacher labels:} leave-one-out KL divergence on Mistral-7B-Instruct output distributions.
    \item \textbf{Loss:} MSE between $\hat{p}_i$ and normalised teacher KL.
    \item \textbf{Pruning:} discard turns where $\hat{p}_i < \tau^* \approx 0.48$ (from S6: 90\% fidelity target).
  \end{itemize}

  \vspace{0.3em}
  \begin{tabular}{lll}
    $\Delta t_{\min} = 0.05$ & $\beta = 1.0$ & $\tau^* \approx 0.48$ \\
    \multicolumn{3}{l}{\small (all three derived from simulations, not hand-tuned)}
  \end{tabular}
\end{frame}

...
\begin{frame}{Why Simulations Before Training?}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}
      ...
      \item If the correlation $\rho \approx 0$, entropy adds no signal and training is wasted.
      \item Hyperparameters $\beta$, $\Delta t_{\min}$, $\tau$ affect faithfulness but are hard to tune end-to-end.
    \end{itemize}

    \column{0.46\textwidth}
    \textbf{The strategy}
    \begin{itemize}
      \item Seven model-free simulations, each answering one design question analytically.
      \item Each simulation outputs a concrete config value or a go/no-go threshold.
      \item Only if all sims pass do we commit to teacher-label generation and CfC training.
    \end{itemize}
  \end{columns}

  \vspace{0.5em}
  \begin{block}{Go/no-go condition (S3)}
    Measure $\rho(\text{surprisal},\,\text{teacher-importance})$ on real QReCC \emph{before} training.
    If $\rho < 0.1$: entropy adds nothing over uniform.
    If $\rho \geq 0.5$: entropy beats cosine similarity.
  \end{block}
\end{frame}
